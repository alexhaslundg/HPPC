...
class System {
public:
    Molecules molecules; //changed this        // all the molecules in the system
...
void BuildNeighborList(System& sys){
    ...
    sys.molecules.neighbours.resize(sys.molecules.no_mol); // Ensure proper allocation

    for(size_t i = 0; i < sys.molecules.no_mol; i++){ 
        sys.molecules.neighbours[i].clear(); // Clear only for molecule i
        
        for (size_t j = 0; j < sys.molecules.no_mol; j++) {
            //if (i == j) continue; // Skip self

            //if (sys.molecules.atoms.size() == 0 || 
            //    sys.molecules.atoms[0].p.size() <= i || 
            //    sys.molecules.atoms[0].p.size() <= j) {
            //    std::cerr << "Out-of-bounds atom access\n";
            //    exit(1);
            //}
        ...
        distances2[i] = 1e99; // Prevent self from being in neighbor list
        ...
        for (size_t j=0; j < target_num; j++) {
            auto& k = index[j];
            if (k < i) {
                if (std::find(sys.molecules.neighbours[k].begin(), sys.molecules.neighbours[k].end(), i) == sys.molecules.neighbours[k].end()) 
...
// toddo: check if this is correct
void UpdateBondForces(System& sys) {
    ...
    for (Bond& bond : molecules.bonds) {
        ...
        for (size_t i = 0; i < molecules.no_mol; i++) {
...
void UpdateAngleForces(System& sys){
    ...
    for (Angle& angle : molecules.angles){
        ...
        //=============================================================
        auto& atom1=molecules.atoms[angle.a1];
        auto& atom2=molecules.atoms[angle.a2];
        auto& atom3=molecules.atoms[angle.a3];
...
System MakeWater(size_t N_molecules){
    ...
    //         mass    ep    sigma charge name //CHANGED: added the number of molecules
    ...
    // we are no longer saving the atoms in a molecule, but in a structure of arrays Oatom is a list of all positions. 
    // question: how do we form molecules from our list of atoms? 
    // std::vector<Atoms> atoms {Oatom, Hatom1, Hatom2};
    //sys.molecules.push_back({atoms, waterbonds, waterangle});
    // TODO: is this correct?
    // System is the variable type. sys is the variable name.
    Molecules molecules({Oatom, Hatom1, Hatom2}, waterbonds, waterangle, N_molecules);
    System sys{molecules};
    // the first argument inside the constructor (for the sys variable) is the first arg. 
